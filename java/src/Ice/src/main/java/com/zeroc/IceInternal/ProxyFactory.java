...
public final class ProxyFactory {
  ...
  public int checkRetryAfterException(
      com.zeroc.Ice.LocalException ex, Reference ref, Holder<Integer> sleepInterval, int cnt) {
    ...
    int interval;
    if (cnt == (_retryIntervals.length + 1)
        && ex instanceof com.zeroc.Ice.CloseConnectionException) {
      //
  ...
  ProxyFactory(Instance instance) {
    ...
    String[] arr =
        _instance.initializationData().properties.getPropertyAsList("Ice.RetryIntervals");
