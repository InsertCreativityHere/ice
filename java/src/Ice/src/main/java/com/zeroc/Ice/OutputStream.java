...
public class OutputStream {
  ...
  /**
   * Releases any data retained by encapsulations. The {@link #reset} method internally calls <code>
   * clear</code>.
   */
  public void clear() {
  ...
  /**
   * Writes a boolean to the stream at the given position. The current position of the stream is not
   * modified.
   *
   * @param v The boolean to write to the stream.
   ...
   *
   * @param v The boolean sequence to write to the stream. Passing <code>null</code> causes an empty
   *     sequence to be written to the stream.
   */
   ...
   *
   * @param v The double sequence to write to the stream. Passing <code>null</code> causes an empty
   *     sequence to be written to the stream.
   */
   ...
   *
   * @param v The string to write to the stream. Passing <code>null</code> causes an empty string to
   ...
   *
   * @param v The string sequence to write to the stream. Passing <code>null</code> causes an empty
   *     sequence to be written to the stream.
   */
  ...
  private static final class EncapsEncoder11 extends EncapsEncoder {
    ...
    void startSlice(String typeId, int compactId, boolean last) {
      ...
      if (last) {
        _current.sliceFlags |=
            com.zeroc.IceInternal.Protocol.FLAG_IS_LAST_SLICE; // This is the last slice.
      }
