...
emitOpNameResult(IceUtilInternal::Output& H, const OperationPtr& p, int useWstring)
{
    ...
    if((outParams.size() > 1) || (ret && outParams.size() > 0))
    {
        ...
        for(ParamDeclList::iterator q = outParams.begin(); q != outParams.end(); ++q)
        {
            string typeString = typeToString((*q)->type(), (*q)->isTagged(), clScope, (*q)->getMetaData(),
                                             useWstring | TypeContextCpp11);
...
Slice::Gen::TypesVisitor::visitExceptionStart(const ExceptionPtr& p)
{
    ...
    {
        string typeName = inputTypeToString((*q)->type(), (*q)->isTagged(), scope, (*q)->getMetaData(), _useWstring);
    ...
    {
        H << getUnqualified("::Ice::UserException", scope);
    }
    H.restoreIndent();
    H << sb;
    ...
    if(!allParamDecls.empty())
    {
        ...
        if(!baseParams.empty() || !dataMembers.empty())
        {
            C << " :";
            C.inc();
            string upcall;
...
Slice::Gen::TypesVisitor::visitStructStart(const StructPtr& p)
{
    ...
    if(!dataMembers.empty() && (findMetaData(p->getMetaData()) == "%class" || p->hasDefaultValues()))
    {
        ...
        {
            string typeName =
                inputTypeToString((*q)->type(), (*q)->isTagged(), scope, (*q)->getMetaData(), _useWstring);
...
Slice::Gen::TypesVisitor::visitDataMember(const DataMemberPtr& p)
{
    ...
    //
    const string scope = "";
    H << nl << typeToString(p->type(), p->isTagged(), scope, p->getMetaData(), _useWstring) << ' ' << name << ';';
}
...
Slice::Gen::TypesVisitor::emitUpcall(const ExceptionPtr& base, const string& call, const string& scope)
{
    ...
    {
        C << getUnqualified("::Ice::UserException", scope);
    }
    C << call;
}
...
usePrivateEnd(const OperationPtr& p)
{
    ...
    for(ParamDeclList::const_iterator q = paramList.begin(); q != paramList.end(); ++q)
    {
        if((*q)->isOutParam())
        {
            outDeclsEnd.push_back(outputTypeToString((*q)->type(), (*q)->isTagged(), "", (*q)->getMetaData(),
                                                     TypeContextAMIEnd));
            outDeclsPrivateEnd.push_back(outputTypeToString((*q)->type(), (*q)->isTagged(), "", (*q)->getMetaData(),
...
Slice::Gen::ProxyVisitor::visitOperation(const OperationPtr& p)
{
    ...
    for(ParamDeclList::const_iterator q = paramList.begin(); q != paramList.end(); ++q)
    {
        ...
        {
            typeString =
                outputTypeToString((*q)->type(), (*q)->isTagged(), "", metaData, _useWstring | TypeContextAMIEnd);
            typeStringEndAMI = outputTypeToString((*q)->type(), (*q)->isTagged(), "", metaData,
                                                  _useWstring | TypeContextAMIPrivateEnd);
        }
        else
        {
            typeString = inputTypeToString((*q)->type(), (*q)->isTagged(), "", metaData, _useWstring);
        }
        ...
        else
        {
            ...
            outDecls.push_back(
                inputTypeToString((*q)->type(), (*q)->isTagged(), "", (*q)->getMetaData(), _useWstring));
...
Slice::Gen::ObjectVisitor::visitOperation(const OperationPtr& p)
{
    ...
    for(ParamDeclList::iterator q = paramList.begin(); q != paramList.end(); ++q)
    {
        ...
        if(isOutParam)
        {
            typeString = outputTypeToString(type, (*q)->isTagged(), classScope, (*q)->getMetaData(), _useWstring);
        }
        else
        {
            typeString =
                inputTypeToString((*q)->type(), (*q)->isTagged(), classScope, (*q)->getMetaData(), _useWstring);
        }
        ...
        {
            outDecls.push_back(inputTypeToString((*q)->type(), (*q)->isTagged(), classScope, (*q)->getMetaData(),
                                                 _useWstring));
        }
    }
...
Slice::Gen::ObjectVisitor::emitDataMember(const DataMemberPtr& p)
{
    ...
    //
    const string scope = "";
    H << nl << typeToString(p->type(), p->isTagged(), scope, p->getMetaData(), _useWstring) << ' '
      << name << ';';
}
...
Slice::Gen::ObjectVisitor::emitOneShotConstructor(const ClassDefPtr& p)
{
    ...
    if(!allDataMembers.empty())
    {
        ...
        {
            string typeName =
                inputTypeToString((*q)->type(), (*q)->isTagged(), scope, (*q)->getMetaData(), typeContext);
...
Slice::Gen::AsyncCallbackTemplateVisitor::generateOperation(const OperationPtr& p, bool withCookie)
{
    ...
    for(ParamDeclList::const_iterator q = paramList.begin(); q != paramList.end(); ++q)
    {
        if((*q)->isOutParam())
        {
            ...
            outDecls.push_back(
                inputTypeToString((*q)->type(), (*q)->isTagged(), clScope, (*q)->getMetaData(), _useWstring));
        }
    }
    ...
    for(OperationList::const_iterator r = ops.begin(); r != ops.end(); ++r)
    {
        ...
        if(p->hasMetaData("amd") || op->hasMetaData("amd"))
        {
            ...
            for(ParamDeclList::const_iterator q = paramList.begin(); q != paramList.end(); ++q)
            {
                if(!(*q)->isOutParam())
                {
                    H << ',' << nl << inputTypeToString((*q)->type(), (*q)->isTagged(), scope, (*q)->getMetaData(),
                                                        _useWstring);
                }
            }
            ...
            for(ParamDeclList::const_iterator q = paramList.begin(); q != paramList.end(); ++q)
            {
                if(!(*q)->isOutParam())
                {
                    C << ',' << nl << inputTypeToString((*q)->type(), (*q)->isTagged(), scope, (*q)->getMetaData(),
        ...
        else
        {
            ...
            for(ParamDeclList::const_iterator q = paramList.begin(); q != paramList.end(); ++q)
            {
                ...
                if((*q)->isOutParam())
                {
                    H << outputTypeToString((*q)->type(), (*q)->isTagged(), scope, (*q)->getMetaData(), _useWstring);
                }
                else
                {
                    H << inputTypeToString((*q)->type(), (*q)->isTagged(), scope, (*q)->getMetaData(), _useWstring);
                }
            }
            ...
            for(ParamDeclList::const_iterator q = paramList.begin(); q != paramList.end(); ++q)
            {
                ...
                if((*q)->isOutParam())
                {
                    C << outputTypeToString((*q)->type(), (*q)->isTagged(), scope, (*q)->getMetaData(), _useWstring) << " "
                      << fixKwd((*q)->name());
                }
                else
                {
                    C << inputTypeToString((*q)->type(), (*q)->isTagged(), scope, (*q)->getMetaData(), _useWstring) << " /*"
                      << fixKwd((*q)->name()) << "*/";
                }
            }
...
Slice::Gen::AsyncVisitor::visitOperation(const OperationPtr& p)
{
    ...
    for(ParamDeclList::const_iterator q = paramList.begin(); q != paramList.end(); ++q)
    {
        string paramName = fixKwd((*q)->name());
        TypePtr type = (*q)->type();
        string typeString = inputTypeToString(type, (*q)->isTagged(), classScope, (*q)->getMetaData(), _useWstring);
...
Slice::Gen::AsyncImplVisitor::visitOperation(const OperationPtr& p)
{
    ...
    for(ParamDeclList::const_iterator q = paramList.begin(); q != paramList.end(); ++q)
    {
        if((*q)->isOutParam())
        {
            string paramName = fixKwd((*q)->name());
            TypePtr type = (*q)->type();
            string typeString = inputTypeToString(type, (*q)->isTagged(), "", (*q)->getMetaData(), _useWstring);
...
Slice::Gen::Cpp11TypesVisitor::visitExceptionStart(const ExceptionPtr& p)
{
    ...
    for(DataMemberList::const_iterator q = allDataMembers.begin(); q != allDataMembers.end(); ++q)
    {
        string typeName = inputTypeToString((*q)->type(), (*q)->isTagged(), scope, (*q)->getMetaData(),
...
Slice::Gen::Cpp11TypesVisitor::visitDataMember(const DataMemberPtr& p)
{
    ...
    string name = fixKwd(p->name());
    writeDocSummary(H, p);
    H << nl << typeToString(p->type(), p->isTagged(), scope, p->getMetaData(), _useWstring | TypeContextCpp11)
      << ' ' << name;
    ...
    if(!defaultValue.empty())
    {
        ...
        if(p->isTagged() && builtin->kind() == Builtin::KindString)
        {
            //
...
Slice::Gen::Cpp11ProxyVisitor::visitOperation(const OperationPtr& p)
{
    ...
    for(ParamDeclList::const_iterator q = paramList.begin(); q != paramList.end(); ++q)
    {
        ...
        if((*q)->isOutParam())
        {
            ...
            // Use empty scope to get full qualified names in types used with future declarations.
            //
            futureOutParams.push_back(typeToString((*q)->type(), (*q)->isTagged(), "", metaData,
                                                   _useWstring | TypeContextCpp11));
            lambdaOutParams.push_back(typeToString((*q)->type(), (*q)->isTagged(), "", metaData,
                                                   _useWstring | TypeContextInParam | TypeContextCpp11));

            string outputTypeString = outputTypeToString((*q)->type(), (*q)->isTagged(), clScope, metaData,
                                                         _useWstring | TypeContextCpp11);

            params.push_back(outputTypeString);
            paramsDecl.push_back(outputTypeString + ' ' + paramName);

            hasTaggedOutParams |= (*q)->isTagged();
        ...
        {
            string typeString = inputTypeToString((*q)->type(), (*q)->isTagged(), clScope, metaData,
                                                  _useWstring | TypeContextCpp11);
...
Slice::Gen::Cpp11ObjectVisitor::emitDataMember(const DataMemberPtr& p)
{
    ...
    writeDocSummary(H, p);
    H << nl << typeToString(p->type(), p->isTagged(), scope, p->getMetaData(), typeContext) << ' ' << name;

    string defaultValue = p->defaultValue();
    if(!defaultValue.empty())
    {
        BuiltinPtr builtin = BuiltinPtr::dynamicCast(p->type());
        if(p->isTagged() && builtin && builtin->kind() == Builtin::KindString)
        {
            //
            // = "<string literal>" doesn't work for optional<std::string>
            //
...
Slice::Gen::Cpp11InterfaceVisitor::visitOperation(const OperationPtr& p)
{
    ...
    for(ParamDeclList::iterator q = paramList.begin(); q != paramList.end(); ++q)
    {
        ...
        if(!isOutParam)
        {
            params.push_back(typeToString(type, (*q)->isTagged(), classScope, (*q)->getMetaData(),
        ...
        else
        {
            ...
            {
                params.push_back(
                    outputTypeToString(type, (*q)->isTagged(), classScope, (*q)->getMetaData(), typeCtx) + " " +
                    paramName);
                args.push_back(condMove(isMovable(type) && !isOutParam, paramPrefix + (*q)->name()));
            }

            string responseTypeS = inputTypeToString((*q)->type(), (*q)->isTagged(), classScope, (*q)->getMetaData(),
                                                     typeCtx);
...
Slice::Gen::Cpp11ImplVisitor::visitClassDefStart(const ClassDefPtr& p)
{
    ...
    for(OperationList::const_iterator r = ops.begin(); r != ops.end(); ++r)
    {
        ...
        if(p->hasMetaData("amd") || op->hasMetaData("amd"))
        {
            ...
            for(ParamDeclList::const_iterator q = inParams.begin(); q != inParams.end(); ++q)
            {
                H << typeToString((*q)->type(), (*q)->isTagged(), scope,
            ...
            else
            {
                ...
                for(ParamDeclList::iterator q = outParams.begin(); q != outParams.end(); ++q)
                {
                    ...
                    responseParams += inputTypeToString((*q)->type(), (*q)->isTagged(), scope, (*q)->getMetaData(),
                                                        _useWstring | TypeContextCpp11);
                }
            }
            ...
            for(ParamDeclList::const_iterator q = inParams.begin(); q != inParams.end(); ++q)
            {
                C << typeToString((*q)->type(), (*q)->isTagged(), scope, (*q)->getMetaData(),
                                  _useWstring | TypeContextInParam | TypeContextCpp11);
                C << ' ' << fixKwd((*q)->name()) << "," << nl;
            }
        ...
        else
        {
            ...
            for(ParamDeclList::const_iterator q = paramList.begin(); q != paramList.end(); ++q)
            {
                ...
                if((*q)->isOutParam())
                {
                    typeString = outputTypeToString((*q)->type(), (*q)->isTagged(), scope, metaData,
                                                    _useWstring | TypeContextCpp11);
                }
                else
                {
                    typeString = typeToString((*q)->type(), (*q)->isTagged(), scope, metaData,
                                              _useWstring | TypeContextInParam | TypeContextCpp11);
                }
                H << typeString;
            }
            ...
            for(ParamDeclList::const_iterator q = paramList.begin(); q != paramList.end(); ++q)
            {
                ...
                if((*q)->isOutParam())
                {
                    C << outputTypeToString((*q)->type(), (*q)->isTagged(), scope, metaData, _useWstring | TypeContextCpp11)
                      << " "
                      << fixKwd((*q)->name());
                }
                else
                {
                    C << typeToString((*q)->type(), (*q)->isTagged(), scope, metaData,
                                      _useWstring | TypeContextInParam | TypeContextCpp11)
                      << " /*" << fixKwd((*q)->name()) << "*/";
                }
            }
