                         ...
                         const string& clScope, int typeCtx, const string& customRet)
{
    ...
    for(ParamDeclList::const_iterator p = params.begin(); p != params.end(); ++p)
    {
        writeParamAllocateCode(out, (*p)->type(), (*p)->isTagged(), clScope, fixKwd(prefix + (*p)->name()),
...
Slice::writeEndCode(Output& out, const ParamDeclList& params, const OperationPtr& op, bool prepend)
{
    ...
    for(ParamDeclList::const_iterator p = params.begin(); p != params.end(); ++p)
    {
        writeParamEndCode(out, (*p)->type(), (*p)->isTagged(), fixKwd(prefix + (*p)->name()), (*p)->getMetaData());
    }
    if(op && op->returnType())
    {
...
Slice::writeMarshalUnmarshalDataMemberInHolder(IceUtilInternal::Output& C,
                                               ...
                                               bool marshal)
{
    writeMarshalUnmarshalCode(C, p->type(), p->isTagged(), p->tag(), holder + fixKwd(p->name()), marshal,
                              p->getMetaData());
}
...
Slice::writeIceTuple(::IceUtilInternal::Output& out, DataMemberList dataMembers, int typeCtx)
{
    ...
    for(DataMemberList::const_iterator q = dataMembers.begin(); q != dataMembers.end(); ++q)
    {
        ...
        out << "const ";
        out << typeToString((*q)->type(), (*q)->isTagged(), scope, (*q)->getMetaData(), typeCtx | TypeContextCpp11)
            << "&";
    }
    out << "> ice_tuple() const";
