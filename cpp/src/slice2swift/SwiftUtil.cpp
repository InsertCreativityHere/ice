                                           ...
                                           bool rootClass)
{
    if(!members.empty())
    {
        ...
        for(DataMemberList::const_iterator i = allMembers.begin(); i != allMembers.end(); ++i)
        {
            DataMemberPtr m = *i;
            out << (fixIdent(m->name()) + ": " +
                    typeToString(m->type(), p, m->getMetaData(), m->isTagged()));
        }
                             ...
                             const ContainedPtr& p)
{
    ...
    for(DataMemberList::const_iterator q = members.begin(); q != members.end(); ++q)
    {
        ...
        string memberType = typeToString(type, p, member->getMetaData(), member->isTagged());
...
SwiftGenerator::operationReturnType(const OperationPtr& op)
{
    ...
    for(ParamDeclList::const_iterator q = outParams.begin(); q != outParams.end(); ++q)
    {
        ...
        os << typeToString((*q)->type(), *q, (*q)->getMetaData(), (*q)->isTagged());
    }
...
SwiftGenerator::getAllInParams(const OperationPtr& op)
{
    ...
    for(ParamDeclList::const_iterator p = l.begin(); p != l.end(); ++p)
    {
        ...
        info.name = (*p)->name();
        info.type = (*p)->type();
        info.typeStr = typeToString(info.type, op, (*p)->getMetaData(), (*p)->isTagged());
        info.isTagged = (*p)->isTagged();
        info.tag = (*p)->tag();
        info.param = *p;
        r.push_back(info);
    }
