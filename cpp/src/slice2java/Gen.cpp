    ...
    else
    {
        ...
        if(!type)
        {
            ...
            if(!outParams.empty())
            {
                ...
                isReturnTagged = outParams.front()->isTagged();
            }
        }
        if(type)
        {
...
Slice::JavaVisitor::writeResultType(Output& out, const OperationPtr& op, const string& package, const CommentPtr& dc)
{
    ...
    for(const auto& p : outParams)
    {
        out << (typeToAnnotatedString(p->type(), TypeModeIn, package, p->getMetaData(), p->isTagged())
                + " " + fixKwd(p->name()));
    }
    out << epar;
    out << sb;
    ...
    for(const auto& p : outParams)
    {
        ...
        out << nl << "public " << typeToAnnotatedString(p->type(), TypeModeIn, package, p->getMetaData(),
            p->isTagged(), false) << ' ' << fixKwd(name) << ';';
    }

    ParamDeclList requiredParams, taggedParams;
...
Slice::JavaVisitor::getParams(const OperationPtr& op, const string& package)
{
    ...
    {
        const string type = typeToAnnotatedString(q->type(), TypeModeIn, package, q->getMetaData(), q->isTagged());
...
Slice::JavaVisitor::writeUnmarshalProxyResults(Output& out, const string& package, const OperationPtr& op)
{
    ...
    else
    {
        ...
        {
            assert(outParams.size() == 1);
            isTagged = outParams.front()->isTagged();
            type = outParams.front()->type();
                                               ...
                                               const string& param)
{
    ...
    else
    {
        ...
        else
        {
            ...
            mode = params.front()->isTagged() ? TaggedOutParam : NotTagged;
    ...
    for(OperationList::const_iterator r = ops.begin(); r != ops.end(); ++r)
    {
        ...
        if(!inParams.empty())
        {
            ...
            for(const auto& pli : inParams)
            {
                ...
                {
                    allocatePatcher(out, paramType, package, "icePP_" + pli->name(), pli->isTagged());
...
Slice::Gen::TypesVisitor::visitDataMember(const DataMemberPtr& p)
{
    ...
    const bool isTagged = p->isTagged();
...
Slice::Gen::ImplVisitor::initResult(Output& out, const string& package, const OperationPtr& op)
{
    ...
    if(op->hasMarshaledResult())
    {
        ...
        for(const auto& p : outParams)
        {
            out << getDefaultValue(package, p->type(), p->isTagged());
        }
        out << "current" << epar << ';';
    }
    else if(op->returnsMultipleValues())
    {
        ...
        for(const auto& p : outParams)
        {
            out << nl << "r." << fixKwd(p->name()) << " = "
                << getDefaultValue(package, p->type(), p->isTagged()) << ';';
            if(p->name() == "returnValue")
            {
                retval = "_returnValue";
            }
